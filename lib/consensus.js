...
export function buildReleases(series, ffEvents = []) {
  ...
  for (const r of byDate.values()) {
    ...
    if (!released || !hasPoint) {
      ...
      // P = nilai FRED terakhir sebelum periode ini (sumber resmi); FF hanya bila FRED kosong.
      const previous = (hasPoint ? points[i].value : lastBefore(points, r.obs)) ?? r.ffPrevious ?? null;
      rows.push({
