...
const TF = {
  "1m": { fetch: "1m", range: "7d", resample: 1 },
  "5m": { fetch: "5m", range: "60d", resample: 1 },
...
const DEMO_VOL = { "1m": 0.00022, "5m": 0.0004, "15m": 0.0006, "30m": 0.0009, "1h": 0.0014, "4h": 0.0024, "1d": 0.0042, "1w": 0.008, "1mo": 0.014 };
...
function generateDemoBars(yahoo, symbolId, tf) {
  ...
  const N = 260;
  const stepMs = { "1m": 60e3, "5m": 5 * 60e3, "15m": 15 * 60e3, "30m": 30 * 60e3, "1h": 60e3 * 60, "4h": 4 * 60e3 * 60, "1d": 86400e3, "1w": 7 * 86400e3, "1mo": 30 * 86400e3 }[tf] || 3600e3;
 ...
 *  Bisa dipakai untuk deteksi pola candlestick / level. */
export async function getBars(symbolId, tf) {
  return loadBars(symbolId, tf);
}
...
export async function getSignal(symbolId, tf, { force = false, skipCooldown = false } = {}) {
  ...
  async function build() {
    const { bars, source } = await loadBars(symbolId, tf);
    ...
    return {
      ...
      series: {
        ...
        ohlc: bars.slice(-N).map((b) => ({
          t: b.t,
          o: Number(b.o.toPrecision(6)),
          h: Number(b.h.toPrecision(6)),
          l: Number(b.l.toPrecision(6)),
          c: Number(b.c.toPrecision(6)),
        })),
      },
    };
  }
