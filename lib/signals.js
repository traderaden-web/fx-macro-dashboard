...
const DEMO_BASE = {
  ...
  "EURUSD=X": 1.092, "GBPUSD=X": 1.285, "USDJPY=X": 156.9, "USDCHF=X": 0.9155,
...
const DEMO_TREND = {
  ...
  "EURUSD=X": 0.3, "GBPUSD=X": 0.38, "USDJPY=X": -0.15, "USDCHF=X": -0.08,
  "AUDUSD=X": 0.42, "USDCAD=X": -0.3, "NZDUSD=X": 0.26, "DX-Y.NYB": -0.12,
};

// Buat bar OHLC deterministik (random walk + drift) untuk (simbol, tf).
// `resample` = jumlah bar mentah yang nanti digabung (4h ← 4× 1h): demo
// dibuat pada interval fetch agar jumlah bar akhir tetap ~260.
function generateDemoBars(yahoo, symbolId, tf, resample = 1) {
  const base = DEMO_BASE[yahoo] || 100;
  const vol = DEMO_VOL[tf] || 0.001;
  const rand = mulberry32(hashSeed(`${symbolId}:${tf}`));
  const N = 260 * resample;
  const fetchStep = { "15m": 15 * 60e3, "30m": 30 * 60e3, "1h": 60e3 * 60, "4h": 60e3 * 60, "1d": 86400e3, "1w": 7 * 86400e3, "1mo": 30 * 86400e3 };
  const stepMs = fetchStep[tf] || 3600e3;
...
/** Ambil bar OHLC yang sudah di-resample untuk (simbol, timeframe).
 *  Di-cache 5 menit per (simbol, tf) — dipakai bersama oleh sinyal, pola,
 *  dan mesin SMC agar satu aset tidak di-fetch berulang kali. */
export async function getBars(symbolId, tf) {
  return cached(`bars:${symbolId}:${tf}`, TTL, () => loadBars(symbolId, tf));
}

/** Muat bar OHLC (yahoo → demo fallback) lalu resample. */
async function loadBars(symbolId, tf) {
  ...
  try {
    ...
    bars = generateDemoBars(yahoo, symbolId, tf, TF[tf].resample);
    source = "demo";
  }
  bars = resampleBars(bars, TF[tf].fetch, TF[tf].resample);
  return { bars, source, symbolId, tf };
}
...
export async function getSignal(symbolId, tf, { force = false, skipCooldown = false } = {}) {
  ...
  async function build() {
    const { bars, source } = await getBars(symbolId, tf);
