...
export async function getSignal(symbolId, tf, { force = false } = {}) {
  ...
  async function build() {
    const { bars: raw } = await fetchCloses(yahoo, tf);
    const bars = resampleBars(raw, TF[tf].fetch, TF[tf].resample);
