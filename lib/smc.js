...
/** Overlay ringan untuk chart Terminal (candle + zona). */
export function detectSmc(ohlc = []) {
  ...
  for (let i = 2; i < n; i++) {
    ...
    if (a.h < c.l) fvg.push({ i0: i - 2, i1: i, top: c.l, bot: a.h, dir: "UP" });
    else if (a.l > c.h) fvg.push({ i0: i - 2, i1: i, top: a.l, bot: c.h, dir: "DOWN" });
  }
  const orderBlocksLite = [];
  for (let i = 3; i < n; i++) {
    const impulse = ohlc[i], prev = ohlc[i - 1];
    const body = Math.abs(impulse.c - impulse.o);
    const rng = impulse.h - impulse.l || 1e-9;
    if (body / rng < 0.55) continue;
    if (impulse.c > impulse.o && prev.c < prev.o) orderBlocksLite.push({ i: i - 1, top: prev.h, bot: prev.l, dir: "UP" });
    else if (impulse.c < impulse.o && prev.c > prev.o) orderBlocksLite.push({ i: i - 1, top: prev.h, bot: prev.l, dir: "DOWN" });
  }
  return {
    swings: swings.slice(-12),
    bos: bos.slice(-8),
    fvg: fvg.slice(-6),
    orderBlocks: orderBlocksLite.slice(-5),
    bias: trend > 0 ? "BULLISH" : trend < 0 ? "BEARISH" : "NETRAL",
  };
}

export function tradeLevels({ signal, price, atr, plan }) {
  if (plan?.mode === "directional") {
    return { entry: plan.entry, sl: plan.sl, tp: plan.tps?.[0]?.price ?? plan.tp1 ?? null };
  }
  if (price == null || atr == null) return { entry: price ?? null, tp: null, sl: null };
  if (signal === "LONG") return { entry: price, tp: price + 1.5 * atr, sl: price - atr };
  if (signal === "SHORT") return { entry: price, tp: price - 1.5 * atr, sl: price + atr };
