...
// Inflasi/GDP ch-jp-ca-au-nz = kurasi Agu 2026 (ONS/STATAT/ABS/StatsCan/RBNZ),
// disinkronkan dengan lib/centralBanks.js (papan /fundamentals).
const D = {
  us: {
  ...
  id: {
    rate: 4.0, inflation: 2.1, gdp: 4.87, unemp: 4.8, move: -25,
    rateBank: "Bank Indonesia (BI-Rate)", gdpNote: "YoY Q2-26 (BPS)",
    moveNote: "−25bp Jun 2026 (4,25 → 4,00); siklus pelonggaran",
  },
  in: {
...
const NAMEMAP = {
  ...
  cn: "Tiongkok", in: "India", id: "Indonesia", br: "Brasil", ru: "Rusia",
};
...
/**
 * Gabungkan data kurasi dengan angka LIVE (FRED) untuk entitas yang seri-nya
 * kami lacak — dipanggil server (app/analysis/page.jsx) dan hasilnya dikirim
 * ke MacroMap sebagai prop `overrides` { us: {rate, inflation, unemp, gdp}, ... }.
 * Nilai null/undefined pada override tidak menimpa nilai kurasi.
 */
export function mergeCountryData(overrides) {
  if (!overrides) return D;
  const out = {};
  for (const k of Object.keys(D)) {
    const o = overrides[k];
    out[k] = o
      ? Object.fromEntries(Object.entries({ ...D[k], ...o }).map(([kk, v]) => [kk, v == null ? D[k][kk] ?? null : v]))
      : D[k];
  }
  return out;
}

// dataFor(idTopo, data?) → { name, bank, rate, inflation, gdp, gdpNote, move, moveNote } | null
export function dataFor(id, data = D) {
  const key = IDMAP[String(id)];
  if (!key) return null;
  const d = data[key] || D[key];
  return {
    key,
    name: NAMEMAP[key],
    bank: d.rateBank,
