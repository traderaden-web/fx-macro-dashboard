...
function friendlyError(e) {
  ...
  if (/failed to fetch|network|fetch failed|ENOTFOUND|ECONN/i.test(raw)) {
    return "Gagal terhubung ke MetaAPI (cek koneksi internet / region / token).";
  }
  if (/HTTP 401|unauthorized|invalid.*token|forbidden/i.test(raw)) {
    return "Token MetaAPI tidak valid atau tidak memiliki akses akun itu.";
  }
  if (/HTTP 404|not found/i.test(raw)) return "Akun MetaAPI tidak ditemukan (cek accountId).";
  if (/timeout/i.test(raw)) return "MetaAPI timeout — coba lagi beberapa saat lagi.";
  return raw;
}
