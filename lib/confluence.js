...
const TF_WEIGHT = { "1m": 0.6, "5m": 0.8, "15m": 1, "30m": 1.2, "1h": 1.5, "4h": 1.8, "1d": 2, "1w": 2.2, "1mo": 2.4 };
...
export function detectRegime(tfs = []) {
  ...
  const strategy =
    type === "TREND"
      ? "Ikuti tren (trend-following): masuk searah tren, letakkan stop di belakang struktur, garap pergerakan panjang."
      : type === "RANGE"
        ? "Mean-reversion: jual di batas atas / beli di batas bawah range, target ke tengah, stop di luar range."
        : "Tunggu konfirmasi (standby): hindari memaksa posisi; tunggu breakout/reversal terkonfirmasi."

  return { type, label, dir, long, short, neutral, count: n, strategy };
}
