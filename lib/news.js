...
async function fetchFeed(feed) {
  ...
  return items
    .map((it) => {
      ...
      // Bisa berisi HTML & entity ter-escape ganda → decode dulu, lalu strip tag,
      // diulang sampai stabil agar tidak ada sisa "&lt;a href=..." di tampilan.
      let descText = get(/<description>([\s\S]*?)<\/description>/) || "";
      for (let i = 0; i < 3; i++) {
        const next = decode(descText)
          .replace(/<[^>]*>/g, " ")
          .replace(/&nbsp;|\u00a0/g, " ")
          .replace(/\s+/g, " ")
          .trim();
        if (next === descText) break;
        descText = next;
      }
      const rssSummary = descText || null;
...
// ── Berita per negara (modal detail Peta Makro) ─────────────────────────
// Query Google News per entitas makro — fokus ekonomi + bank sentral negara.
const COUNTRY_QUERIES = {
  us: "United States economy OR \"Federal Reserve\" OR Fed rate",
  euro: "eurozone OR \"euro area\" economy OR ECB",
  gb: "\"United Kingdom\" economy OR \"Bank of England\"",
  ch: "Switzerland economy OR \"Swiss National Bank\"",
  jp: "Japan economy OR BOJ OR \"Bank of Japan\"",
  ca: "Canada economy OR \"Bank of Canada\"",
  au: "Australia economy OR \"Reserve Bank of Australia\" OR RBA",
  nz: "New Zealand economy OR \"Reserve Bank of New Zealand\"",
  cn: "China economy OR \"People's Bank of China\" OR PBoC",
  in: "India economy OR \"Reserve Bank of India\" OR RBI",
  br: "Brazil economy OR \"central bank\"",
  ru: "Russia economy OR \"central bank\"",
};

/** Berita terkini per negara makro — cache 10 menit per negara. */
export async function getCountryNews(key, n = 8) {
  const q = COUNTRY_QUERIES[key];
  if (!q) throw new Error(`Negara ${key} tidak dikenal`);
  const items = await cached(`news:country:${key}`, TTL, async () => {
    const list = await fetchFeed({ key: `country:${key}`, cat: "negara", q });
    return list.sort((a, b) => (b.iso || "").localeCompare(a.iso || "")).slice(0, n);
  });
  return { items, source: "live", updated: new Date().toISOString() };
}
