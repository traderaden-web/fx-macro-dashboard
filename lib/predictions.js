...
/** Rilis penting yang dibuka untuk prediksi komunitas — diturunkan OTOMATIS
 *  dari jadwal resmi (data/calendar.js): rilis NFP, CPI, dan FOMC berikutnya
 *  yang belum lewat. `expected` (konsensus) diisi oleh route API dari
 *  lib/consensus (ForexFactory) bila tersedia. */
const OPEN_DEFS = [
  { indicatorId: "nfp", title: "Nonfarm Payrolls (NFP)", unit: "ribu", match: /Nonfarm Payrolls/ },
  { indicatorId: "cpi", title: "CPI y/y (AS)", unit: "%", match: /Consumer Price Index/ },
  { indicatorId: "fedfunds", title: "FOMC Federal Funds Rate", unit: "%", match: /FOMC Federal Funds Rate/ },
];

export function openEvents(now = new Date()) {
  const t = now.getTime();
  return OPEN_DEFS.map((d) => {
    const next = UPCOMING
      .filter((e) => e.indicatorId === d.indicatorId && d.match.test(e.title) && new Date(e.iso).getTime() > t)
      .sort((a, b) => a.iso.localeCompare(b.iso))[0];
    if (!next) return null;
    return { indicatorId: d.indicatorId, releaseDate: next.date, releaseIso: next.iso, title: d.title, unit: d.unit, expected: null };
  }).filter(Boolean);
}

/** Kompatibilitas: snapshot saat modul dimuat (route API memakai openEvents()). */
export const OPEN_EVENTS = openEvents();
