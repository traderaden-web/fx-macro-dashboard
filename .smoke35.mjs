...
add("judul popup: INDIKATOR — TGL", /CPI — [A-Z]{3} \d{2}/.test(head), head);
...
add("konteks akurasi konsensus", /KONTEKS AKURASI KONSENSUS/.test(modal()?.textContent || ""));
...
add("tabel estimasi pair ada", modal()?.querySelectorAll(".rm-sec .ct-table tbody tr").length > 0, modal()?.querySelectorAll(".rm-sec .ct-table tbody tr").length);
