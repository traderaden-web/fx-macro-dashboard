...
const nfpDec = nfpRows.find((tr) => tr.textContent.includes("9 JAN 26"));
const nfpDecCells = nfpDec ? [...nfpDec.querySelectorAll("td")].map((td) => td.textContent.trim()) : [];
add("NFP: baris 9 JAN 26 (data Des) P=56 K=66 A=48 (FF Jan)", nfpDecCells.includes("56") && nfpDecCells.includes("66") && nfpDecCells.includes("48"), nfpDecCells.join("|"));
const nfpJan = nfpRows.find((tr) => tr.textContent.includes("11 FEB 26"));
const nfpJanCells = nfpJan ? [...nfpJan.querySelectorAll("td")].map((td) => td.textContent.trim()) : [];
add("NFP: baris 11 FEB 26 (data Jan) P=48 K=66 A=126 (FF Feb)", nfpJanCells.includes("48") && nfpJanCells.includes("66") && nfpJanCells.includes("126"), nfpJanCells.join("|"));
const nfpFeb = nfpRows.find((tr) => tr.textContent.includes("6 MAR 26"));
const nfpFebCells = nfpFeb ? [...nfpFeb.querySelectorAll("td")].map((td) => td.textContent.trim()) : [];
add("NFP: baris 6 MAR 26 (data Feb) P=126 K=58 A=−133 (FF Mar)", nfpFebCells.includes("126") && nfpFebCells.includes("58") && nfpFebCells.some((v) => v.replace(/[−-]/g,"") === "133"), nfpFebCells.join("|"));
...
const mfgJan = mfgRows.find((tr) => tr.textContent.includes("2 FEB 26"));
const mfgJanCells = mfgJan ? [...mfgJan.querySelectorAll("td")].map((td) => td.textContent.trim()) : [];
add("ISM Mfg: baris 2 FEB 26 (data Jan) P=47,9 K=48,5 A=52,6 (FF Feb)", mfgJanCells.includes("47,9") && mfgJanCells.includes("48,5") && mfgJanCells.includes("52,6"), mfgJanCells.join("|"));
const mfgDec = mfgRows.find((tr) => tr.textContent.includes("5 JAN 26"));
const mfgDecCells = mfgDec ? [...mfgDec.querySelectorAll("td")].map((td) => td.textContent.trim()) : [];
add("ISM Mfg: baris 5 JAN 26 (data Des) P=48,2 K=48,3 A=47,9 (FF Jan)", mfgDecCells.includes("47,9") && mfgDecCells.includes("48,3") && mfgDecCells.includes("48,2"), mfgDecCells.join("|"));
const mfgFeb = mfgRows.find((tr) => tr.textContent.includes("2 MAR 26"));
const mfgFebCells = mfgFeb ? [...mfgFeb.querySelectorAll("td")].map((td) => td.textContent.trim()) : [];
add("ISM Mfg: baris 2 MAR 26 (data Feb) P=52,6 K=51,7 A=52,4 (FF Mar)", mfgFebCells.includes("52,4") && mfgFebCells.includes("51,7") && mfgFebCells.includes("52,6"), mfgFebCells.join("|"));
...
const svcApr = svcRows.find((tr) => tr.textContent.includes("5 MEI 26"));
const svcAprCells = svcApr ? [...svcApr.querySelectorAll("td")].map((td) => td.textContent.trim()) : [];
add("ISM Svc: baris 5 MEI 26 (data Apr) P=54 K=53,7 A=53,6 (FF Apr)", svcAprCells.includes("53,6") && svcAprCells.includes("53,7") && svcAprCells.includes("54"), svcAprCells.join("|"));
const svcDec = svcRows.find((tr) => tr.textContent.includes("7 JAN 26"));
const svcDecCells = svcDec ? [...svcDec.querySelectorAll("td")].map((td) => td.textContent.trim()) : [];
add("ISM Svc: baris 7 JAN 26 (data Des) P=52,6 K=52,2 A=54,4 (FF Jan)", svcDecCells.includes("54,4") && svcDecCells.includes("52,2") && svcDecCells.includes("52,6"), svcDecCells.join("|"));
const svcJan = svcRows.find((tr) => tr.textContent.includes("4 FEB 26"));
const svcJanCells = svcJan ? [...svcJan.querySelectorAll("td")].map((td) => td.textContent.trim()) : [];
add("ISM Svc: baris 4 FEB 26 (data Jan) P=54,4 K=53,5 A=53,8 (FF Feb)", svcJanCells.includes("53,8") && svcJanCells.includes("53,5") && svcJanCells.includes("54,4"), svcJanCells.join("|"));
const svcFeb = svcRows.find((tr) => tr.textContent.includes("4 MAR 26"));
const svcFebCells = svcFeb ? [...svcFeb.querySelectorAll("td")].map((td) => td.textContent.trim()) : [];
add("ISM Svc: baris 4 MAR 26 (data Feb) P=53,8 K=53,5 A=56,1 (FF Mar)", svcFebCells.includes("56,1") && svcFebCells.includes("53,5") && svcFebCells.includes("53,8"), svcFebCells.join("|"));
...
const pceMar2 = pceRows.find((tr) => tr.textContent.includes("30 APR 26"));
const pceMar2Cells = pceMar2 ? [...pceMar2.querySelectorAll("td")].map((td) => td.textContent.trim()) : [];
add("Core PCE: baris 30 APR 26 (data Mar) P=0,4 K=0,3 A=0,3 (FF Apr)", pceMar2Cells.includes("0,3") && pceMar2Cells.includes("0,4"), pceMar2Cells.join("|"));
const pceDec = pceRows.find((tr) => tr.textContent.includes("22 JAN 26"));
const pceDecCells = pceDec ? [...pceDec.querySelectorAll("td")].map((td) => td.textContent.trim()) : [];
add("Core PCE: baris 22 JAN 26 (data Nov) P=0,2 K=0,2 A=0,2 (FF Jan)", pceDecCells.filter((v) => v === "0,2").length >= 3, pceDecCells.join("|"));
const pceJan = pceRows.find((tr) => tr.textContent.includes("20 FEB 26"));
const pceJanCells = pceJan ? [...pceJan.querySelectorAll("td")].map((td) => td.textContent.trim()) : [];
add("Core PCE: baris 20 FEB 26 (data Des) P=0,2 K=0,3 A=0,4 (FF Feb)", pceJanCells.includes("0,4") && pceJanCells.includes("0,3") && pceJanCells.includes("0,2"), pceJanCells.join("|"));
const pceFeb = pceRows.find((tr) => tr.textContent.includes("13 MAR 26"));
const pceFebCells = pceFeb ? [...pceFeb.querySelectorAll("td")].map((td) => td.textContent.trim()) : [];
add("Core PCE: baris 13 MAR 26 (data Jan) P=0,4 K=0,4 A=0,4 (FF Mar)", pceFebCells.filter((v) => v === "0,4").length >= 3, pceFebCells.join("|"));
...
add("ADP: baris 1 APR 26 (data Feb) P=66 K=41 A=66 (terkini; rilis awal 62)", adpMarCells.includes("66") && adpMarCells.includes("41"), adpMarCells.join("|"));
const adpNov = adpRows.find((tr) => tr.textContent.includes("7 JAN 26"));
const adpNovCells = adpNov ? [...adpNov.querySelectorAll("td")].map((td) => td.textContent.trim()) : [];
add("ADP: baris 7 JAN 26 (data Nov) P=29 K=49 A=37 (FF Jan)", adpNovCells.includes("29") && adpNovCells.includes("49") && adpNovCells.includes("37"), adpNovCells.join("|"));
const adpDec = adpRows.find((tr) => tr.textContent.includes("4 FEB 26"));
const adpDecCells = adpDec ? [...adpDec.querySelectorAll("td")].map((td) => td.textContent.trim()) : [];
add("ADP: baris 4 FEB 26 (data Des) P=37 K=46 A=11 (FF Feb)", adpDecCells.includes("37") && adpDecCells.includes("46") && adpDecCells.includes("11"), adpDecCells.join("|"));
const adpJan = adpRows.find((tr) => tr.textContent.includes("4 MAR 26"));
const adpJanCells = adpJan ? [...adpJan.querySelectorAll("td")].map((td) => td.textContent.trim()) : [];
add("ADP: baris 4 MAR 26 (data Jan) P=11 K=50 A=66 (FF Mar)", adpJanCells.includes("11") && adpJanCells.includes("50") && adpJanCells.includes("66"), adpJanCells.join("|"));
...
const cpiJan = cpiRows.find((tr) => tr.textContent.includes("13 FEB 26"));
const cpiJanCells = cpiJan ? [...cpiJan.querySelectorAll("td")].map((td) => td.textContent.trim()) : [];
add("CPI: baris 13 FEB 26 (data Jan) P=2,7 K=2,5 A=2,4 (FF Feb)", cpiJanCells.includes("2,4") && cpiJanCells.includes("2,5") && cpiJanCells.includes("2,7"), cpiJanCells.join("|"));
const cpiFebR = cpiRows.find((tr) => tr.textContent.includes("11 MAR 26"));
const cpiFebRCells = cpiFebR ? [...cpiFebR.querySelectorAll("td")].map((td) => td.textContent.trim()) : [];
add("CPI: baris 11 MAR 26 (data Feb) P=2,4 K=2,4 A=2,4 (FF Mar)", cpiFebRCells.filter((v) => v === "2,4").length >= 2, cpiFebRCells.join("|"));
...
const retailApr = retailRows.find((tr) => tr.textContent.includes("1 APR 26") && !tr.textContent.includes("21 APR 26"));
const retailAprCells = retailApr ? [...retailApr.querySelectorAll("td")].map((td) => td.textContent.trim()) : [];
add("Retail: baris 1 APR 26 (data Mar) P=−0,1 K=0,5 A=0,6 (FF Apr)", retailAprCells.includes("0,6") && retailAprCells.includes("0,5") && retailAprCells.some((v) => v.replace(/[−-]/g,"") === "0,1"), retailAprCells.join("|"));
const retailMay = retailRows.find((tr) => tr.textContent.includes("14 MEI 26"));
const retailMayCells = retailMay ? [...retailMay.querySelectorAll("td")].map((td) => td.textContent.trim()) : [];
add("Retail: baris 14 MEI 26 (data Apr) P=1,6 K=0,5 A=0,5 (FF Apr)", retailMayCells.includes("1,6") && retailMayCells.includes("0,5"), retailMayCells.join("|"));
const retailJun = retailRows.find((tr) => tr.textContent.includes("16 JUN 26"));
const retailJunCells = retailJun ? [...retailJun.querySelectorAll("td")].map((td) => td.textContent.trim()) : [];
add("Retail: baris 16 JUN 26 (data Jun) P=0,5 K=0,7 A=0,2 (FF Jun)", retailJunCells.includes("0,2") && retailJunCells.includes("0,7") && retailJunCells.includes("0,5"), retailJunCells.join("|"));
const retailJan = retailRows.find((tr) => tr.textContent.includes("10 FEB 26"));
const retailJanCells = retailJan ? [...retailJan.querySelectorAll("td")].map((td) => td.textContent.trim()) : [];
add("Retail: baris 10 FEB 26 (data Jan) P=0,6 K=0,4 A=−0,1 (FF Feb; Jan revisi 0,0→−0,1)", retailJanCells.includes("0,6") && retailJanCells.includes("0,4") && retailJanCells.some((v) => v.replace(/[−-]/g, "") === "0,1"), retailJanCells.join("|"));
const retailFeb = retailRows.find((tr) => tr.textContent.includes("6 MAR 26"));
const retailFebCells = retailFeb ? [...retailFeb.querySelectorAll("td")].map((td) => td.textContent.trim()) : [];
add("Retail: baris 6 MAR 26 (data Feb) P=−0,1 K=−0,3 A=−0,1 (FF Mar; P = Jan revisi)", retailFebCells.some((v) => v.replace(/[−-]/g,"") === "0,3") && retailFebCells.filter((v) => v.replace(/[−-]/g,"") === "0,1").length >= 2, retailFebCells.join("|"));

const umichRow = $$(".ct-side .ct-row").find((b) => /Michigan|UoM/.test(b.textContent));
add("picker: UoM tersedia", !!umichRow, $$(".ct-side .ct-row").map((b) => b.textContent.slice(0, 10)).join("|").slice(0, 160));
await click(umichRow);
await act(async () => { await new Promise((r) => setTimeout(r, 500)); });
const umichRows = rows();
const umichJan = umichRows.find((tr) => tr.textContent.includes("6 FEB 26"));
const umichCells = umichJan ? [...umichJan.querySelectorAll("td")].map((td) => td.textContent.trim()) : [];
add("UoM: baris 6 FEB 26 (data Jan) P=56,4 K=55 A=57,3 (FF Feb)", umichCells.includes("57,3") && umichCells.includes("55") && umichCells.includes("56,4"), umichCells.join("|"));
const umichDec = umichRows.find((tr) => tr.textContent.includes("9 JAN 26"));
const umichDecCells = umichDec ? [...umichDec.querySelectorAll("td")].map((td) => td.textContent.trim()) : [];
add("UoM: baris 9 JAN 26 (data Des) P=52,9 K=53,5 A=56,4 (FF Jan; Des revisi 54,0→56,4)", umichDecCells.includes("56,4") && umichDecCells.includes("53,5") && umichDecCells.includes("52,9"), umichDecCells.join("|"));
const umichApr = umichRows.find((tr) => tr.textContent.includes("10 APR 26"));
const umichAprCells = umichApr ? [...umichApr.querySelectorAll("td")].map((td) => td.textContent.trim()) : [];
add("UoM: baris 10 APR 26 (data Mar) P=56,6 K=51,6 A=47,6 (FF Apr)", umichAprCells.includes("47,6") && umichAprCells.includes("51,6") && umichAprCells.includes("56,6"), umichAprCells.join("|"));
const umichMay = umichRows.find((tr) => tr.textContent.includes("8 MEI 26"));
const umichMayCells = umichMay ? [...umichMay.querySelectorAll("td")].map((td) => td.textContent.trim()) : [];
add("UoM: baris 8 MEI 26 (data Apr) P=47,6 K=49,7 A=48,2 (FF Apr)", umichMayCells.includes("48,2") && umichMayCells.includes("49,7") && umichMayCells.includes("47,6"), umichMayCells.join("|"));
const umichJun = umichRows.find((tr) => tr.textContent.includes("12 JUN 26"));
const umichJunCells = umichJun ? [...umichJun.querySelectorAll("td")].map((td) => td.textContent.trim()) : [];
add("UoM: baris 12 JUN 26 (data Mei) P=48,2 K=46,1 A=48,9 (FF Jun)", umichJunCells.includes("48,9") && umichJunCells.includes("46,1") && umichJunCells.includes("48,2"), umichJunCells.join("|"));
